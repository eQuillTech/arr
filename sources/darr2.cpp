...
{
	if(!isSquare()){arr_err=1;return 0;}
...
double darr2::findAngle(const size_t i,const size_t j,double &ang) const
{
    ...
    double Aqq=at(i,j);
		...
		if(tau>=0.)t=1./(tau+::sqrt(1.+::sqr(tau)));
    else t=-1./(-tau+::sqrt(1.+::sqr(tau)));
...
darr2 darr2::Householder(const size_t i) const
{
		...
		en(0)=1.;
		darr1 xn(m-i);
		for(size_t j=0;j<m-i;j++)xn(j)=at(i+j,i);
		double rho=-sgn(xn(0));if(rho==0.)rho=1.;
		double xmag=xn.mag();
		darr1 u=xn;
		u(0)-=rho*xmag;
		if(u.mag()>0.)
...
darr2 darr2::Householder(const size_t iRow,const size_t jCol) const
{
		...
		en(0)=1.;
		darr1 xn(m-iRow);
		for(size_t j=0;j<m-iRow;j++)xn(j)=at(iRow+j,jCol);
		double rho=-sgn(xn(0));if(rho==0.)rho=1.;
		double xmag=xn.mag();
		darr1 u=xn;
		u(0)-=rho*xmag;
...
//Return Hessenberg (tridiagonal) form of symmetric matrix using unitary transformation P
//*this=P.T()*H*P
...
darr2 darr2::upperTriang(darr2 &P) const
{
	...
	if(m_nRows>m_nCols)nRows=m_nCols;
	P=darr2::ident(m_nRows,m_nRows);
	darr2 R=A;
...
//Multiplies *this by rotation matrix from right affecting cols iCol and jCol
...
//Multiplies *this by rotation matrix from left affecting rows iRow and jRow
...
//Generate matrix with cols representing nullspace basis vector ((*this)*B.T() = 0*B.T());
