...
dbl2 dbl2::operator/(double x) const { return (1. / x) * (*this); }

dbl2 dbl2::operator*=(double x) { return *this = x * (*this); }
...
{
    dbl2 AT = T();
    dbl2 res = (AT * (*this)).inv() * AT;
    return res;
}

//
dbl2 dbl2::pseudoInvRight() const
{
    dbl2 AT = T();
    dbl2 AAT = (*this) * AT;
    dbl2 AATi = AAT.inv();
...
//
dbl2 dbl2::dampedPseudoInvLeft(double lambda) const
{
    dbl2 AT = T();
    dbl2 ATA = AT * (*this);
    dbl2 G = ATA + lambda * (ATA.diag()).diag();
    dbl2 Gi = G.inv();
    dbl2 res = Gi * AT;
    return res;
}
...
//
dbl2 dbl2::adjugate() const { return cofactor().T(); }

//
dbl2 dbl2::makePivotable(bool& isSingular) const
{
    dbl2 A(*this);
    std::size_t m = size(0);
    std::size_t i = 0, j;
...
//
dbl2 dbl2::upper(bool& isSingular) const
{
    isSingular = false;
    dbl2 A(*this); //=makePivotable(isSingular);
...
bool dbl2::PLU(dbl2& P, dbl2& L, dbl2& U) const
{
    std::size_t m = size(0), n = size(1);
...
//
double dbl2::findAngle(const size_t i, const size_t j, double& ang) const
{
    double s, c;
    double Apq = at(i, j);
    if (Apq != 0.) {
...
dbl2 dbl2::Householder(const size_t i) const
{
    size_t m = size(0);
    dbl2 H = dbl2::ident(m, m);
    dbl1 en = dbl1::zero(m - i);
    en(0) = 1.;
    dbl1 xn(m - i);
    ...
    if (u.mag() > 0.) {
        dbl1 v = u.norm();
        dbl2 vvT = v * v.T();
        dbl2sub(H(i, i, m - i, m - i)) -= 2. * vvT;
    }
    return H;
}

//
dbl2 dbl2::Householder(const size_t iRow, const size_t jCol) const
{
    size_t m = size(0);
    dbl2 H = dbl2::ident(m, m);
    dbl1 en = dbl1::zero(m - iRow);
    en(0) = 1.;
    dbl1 xn(m - iRow);
...
dbl2 dbl2::tridiag(dbl2& P) const
{
    if (!isSquare())
        return *this;
...
//
dbl2 dbl2::QR(dbl2& Q) const
{
    dbl2 QT;
    dbl2 R = upperTriang(QT);
    Q = QT.T();
    return R;
}

//
bool dbl2::diagSymm(dbl2& s, dbl2& V) const
{
    if (!isSquareSymm())
        return false;
...
//
dbl2 dbl2::diagSymm(dbl2& v) const
{
    dbl2 s;
...
// Iterate
bool dbl2::SVD(dbl2& U, dbl2& S, dbl2& V) const
{
    bool res = false;
    dbl2 A(*this);
...
//
dbl2 dbl2::pseudoInv(double fac) const
{
    dbl2 U, S, V;
    if (SVDred(U, S, V, fac))
        return V * S.inv() * U.T();
...
//
dbl2 dbl1::T() const { return dbl2(*this).T(); }

//
dbl2 dbl2::zero(const size_t m, const size_t n)
{
    dbl2 res(m, n);
    res = 0.;
    return res;
...
// static
dbl2 dbl2::ident(const size_t nRows, const size_t nCols)
{
    dbl2 A(nRows, nCols);
    for (size_t i = 0; i < nRows; i++)
...
//
dbl2 dbl2::rot(const size_t iRow, const size_t jCol, double ang, size_t n)
{
    dbl2 A = ident(n, n);
...
dbl2 dbl2::span() const
{
    dbl2 A = rowEschelon().T();
    size_t nRows = A.size(0), nCols = A.size(1);
    dbl2 B = ident(nRows, nRows);
