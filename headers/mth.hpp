...
//
double pwr(const double x, const size_t y) { return pow(x, y); }
...
//
double rndom() { return 1. / (RAND_MAX + 1.) * rand(); }

//
double Kdelta(size_t i, size_t j) { return (i == j) ? 1 : 0; }
...
double dmod(const double x, const double y) { return x - y * floor(x / y); }
